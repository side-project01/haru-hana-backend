...
- 해결(Phase 2): User 모델 추가 후 `AnonIdGuard`에 `PrismaService` 주입, `canActivate`를 async로 전환해 `prisma.user.upsert`로 lastSeen 갱신. 지표 갱신은 비핵심이라 실패를 try/catch로 삼켜 요청을 막지 않음(로그에 anonId 원문 미노출). 이월된 lastSeen 테스트(갱신 호출/실패 비차단) 2건 추가.

## [Phase 2] 인프라 미비 — 로컬 DB 미기동으로 migrate dev 미실행
- 상황: `npx prisma migrate dev --name add-user-question` 실행 시 `P1001: Can't reach database server at localhost:5432`. 로컬 PostgreSQL이 기동돼 있지 않음.
- 영향/위험: `prisma/migrations/` 마이그레이션 파일이 생성되지 않았고 실제 DB 스키마는 미반영. 빌드/단위 테스트는 Prisma를 mock 하므로 영향 없음.
